...
class ConcatReader(object):
    ...
    def __getitem__(self, idx):
        ...
        #     l_f = len(self.readers[0])
        # else:
        #     l_f = len(self.intersection_file_name)
        # if idx < l_f:
        #     for r in self.readers:
        #         if isinstance(r, SubVideoReader):
        #             content.update(r.get_file())
        #         else:
...
class ImageReader(Reader):
    ...
    def get_content(self, name):
        ...
        if img is None:
            warnings.warn(os.path.join(self.path, name) + " is not an image!")
            return {"info": {}, "image": None}
        h, w = img.shape[:2]
        content = {"image": img,
                   "info": {"imageName": name,
                            "imageWidth": w,
...
class VideoReader(Reader):
    ...
    def __len__(self):
        return sum([len(d) for d in self.sub_readers])

    def get_name(self, item):
        return self.filenames[item]

    def get_file(self, *args, **kwargs):
        reader_idx = bisect.bisect_right(self.cumulative_sizes, self.idx)
        if reader_idx == 0:
            sample_idx = self.idx
        else:
            sample_idx = self.idx - self.cumulative_sizes[reader_idx - 1]
            if self.pre_reader_idx != reader_idx:
                self.sub_readers[self.pre_reader_idx].close()
                self.pre_reader_idx = reader_idx
        content = self.sub_readers[reader_idx][sample_idx]
        self.idx += 1
        # while content is None and self.idx < self.cumulative_sizes[-1]:
        #     content = self.get_file(*args, **kwargs)
        return content
...
class SubVideoReader(Reader):
    ...
    def __len__(self):
        l = math.ceil(self.length / self.frameRate)
        return int(l)
    ...
    def __next__(self):
        while self.cap.isOpened():
            ...
            if ret and self.idx < self.length:
    ...
    def _gen_content(self, img, idx):
        name = self.video_name + "_" + str(idx).rjust(6, "0") + ".jpg"
        content = {"image": img,
                   "info": {"imageName": name,
