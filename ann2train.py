...
labels = {"gansha": 1, "kailie": 2, "huangban": 3, "xuhua": 4, "wuwu": 5, "bujun": 6, "wanzhe": 7}


def save_mask(target_path, tar_name, anno_content):
    mask = np.zeros((3648, 5472))
    for dict in anno_content:
        points = dict["points"]
        n = len(points)
        points = np.array(points, dtype=np.int32)
        cv2.fillPoly(mask, np.int32([points]), color=labels[dict["label"]])
    cv2.imwrite(os.path.join(target_path, tar_name + ".png"), mask)
...
class Json2Yolov4(object):
    ...
    def convert_json_annotation(self, ain_path, file, src_name, tar_name):
        ...
        save_mask(self.target_path, tar_name, anno_content)
...
def main(ain_path, target_path, classes_dict, ratio, mod):
    json2yolov4 = Json2Yolov4(ain_path, target_path, classes_dict, ratio, mod)
...
if __name__ == '__main__':
    ...
    ain_path = "/home/xq/文档/projects/zhongji/datasets/标注/0412"
    target_path = "/home/xq/文档/projects/zhongji/datasets/标注/0412_train"
    # classes_dict = None
    mod = "json2mask"
